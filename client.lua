...
addCommandHandler("resetdebug",function()
    ...
    outputChatBox("Debugscript sikeresen alap pozícióra helyezve!",0,200,0);
end);

addCommandHandler("cleardebugscript",function()
    cache = {};
    table.insert(cache,1,{3,1,"SIkeresen kiürítetted a debugscriptet.","client",1});
end);

addCommandHandler("debug",function()
    debugShow = not debugShow;
    if debugShow then 
        outputChatBox("Sikeresen bekapcsoltad a debugscriptet!",0,200,0);
        removeEventHandler("onClientRender",root,debugRender);
        addEventHandler("onClientRender",root,debugRender);
    else 
        outputChatBox("Sikeresen bekapcsoltad a debugscriptet!",200,0,0);
        removeEventHandler("onClientRender",root,debugRender);
    end
end);
